...
// ------------------------------------
export async function findEntitysNotations(info) {
    ...
    if (isInfoAsDtdInfo) {
        ...
        if (subsetText)
            matchEntityNotation(subsetText);
        // recursive
        if (info.hasExternal && info.systemId) {
            return fetch(info.systemId)
...
// ]
// }
export async function notationXmlToObject(xmlText) {
    ...
    while ((match = notationRegex.exec(xmlText)) !== null) {
        ...
        let attrMatch;
        const obj = { name: "", publicId: "", systemId: "" };
...
async function validate(data, stopOnFailure) {
    const bags = []; // berupa nama notasi atau entity
...
export async function validateEntityNotation(xmlText, stopOnFailure = true) {
