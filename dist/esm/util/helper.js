 ...
 */
export function mergeByBasenameKeepFullPath(existingArr, newItems) {
 ...
 * @returns
 */
export function resolveUriToBase(uri) {
 ...
 */
export function resolveUri(uri, baseUri) {
    try {
 ...
 * @returns xml text
 */
export async function getSchemaText(file) {
    if (isXmlLike(file)) {
 ...
 */
export function detectDtdLocation(xmlText) {
    const doctypeRegex = /<!DOCTYPE\s+([a-zA-Z0-9:_-]+)\s*((?:PUBLIC\s+"[^"]+"\s+"[^"]+")|(?:PUBLIC\s+"[^"]+")|(?:SYSTEM\s+"[^"]+")|)\s*(?:\[(.*?)\])?\s*>/is;
 ...
 */
export async function findRequiredDtds(mainDtdUrl, visited = new Set()) {
    try {
    ...
    return fetch(mainDtdUrl)
        .then((res) => {
        if (!res.ok)
            throw new Error(`Gagal fetch DTD: ${mainDtdUrl}`);
        return res.text();
    })
        .then(async (text) => {
        const nestedUrls = [];
        // DETEKSI ENTITY SYSTEM "xxx.dtd" dan parameter entity %name SYSTEM...
        const entityRegex = /<!ENTITY\s+(?:%?\s*\w+)\s+SYSTEM\s+"([^"]+)"/gi;
        ...
        const notationRegex = /<!NOTATION\s+\w+\s+(?:SYSTEM\s+"([^"]+)"|PUBLIC\s+"[^"]+"\s+"([^"]+)")/gi;
        ...
        return Promise.all(nestedUrls.map((url) => findRequiredDtds(url, visited)))
            .then((nested) => {
...
export function constructEntityNotationValidationOption(allowedNotation, entityValidNotation = true, notationValidName = true, notationValidPublicId = true) {
    return {
        entity: {
